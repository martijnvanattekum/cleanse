...
genes_subset_se <- seq_se %>% filter(col, time == 0) %>% select(col, -time)
print_options(genes_subset_se)  #note the time variable has disappeared from the colData

# rename the time variable after changing it to minutes
seq_se %>% mutate(col, time = (time * 60)) %>% rename(col, time_mins = time)
print_options(genes_subset_se)  #note the time variable is now called time_mins
```
